...
class TaskCard(QFrame):
    ...
    def resizeEvent(self, event):
        super().resizeEvent(event)
        # Position overlay buttons (done/refuse) on the right side with custom Y
        x = self.width() - 44
        if hasattr(self, 'done_btn') and self.done_btn:
            self.done_btn.setGeometry(x + 4, 6, 35, 35)    # ← Y = 6
        if hasattr(self, 'refuse_btn') and self.refuse_btn:
            self.refuse_btn.setGeometry(x + 4, 40, 35, 35)  # ← Y = 40
    ...
    def _build_ui(self):
        ...
        # Absolute-positioned overlay buttons (done/refuse) with independent Y control
        ...
        self.refuse_btn = QPushButton(self)
